...
const server=createServer(async(req,res)=>{
  try{
    ...
    const response=await handleRequest(request,{db:connection.db,mode:connection.mode,origins:settings.origins,secureCookies:settings.secureCookies,clientAddress:req.socket.remoteAddress??'unknown'});
    res.writeHead(response.status,Object.fromEntries(response.headers));res.end(Buffer.from(await response.arrayBuffer()));
  }catch{res.writeHead(500,{'Content-Type':'application/json'});res.end('{"error":"Request failed."}')}
});
server.requestTimeout=15000;server.headersTimeout=10000;
server.listen(settings.port,'127.0.0.1',()=>console.log(`OnTime API http://127.0.0.1:${settings.port} — storage: ${connection.mode}${connection.mode==='local'?' (persistent local PostgreSQL; not Tiger Data)':''}`));
